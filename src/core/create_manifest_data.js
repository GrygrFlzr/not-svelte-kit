...
export default function create_manifest_data(config, extensions = '.svelte') {
    ...
    const check_pattern = (pattern, file) => {
        ...
        if (seen.has(pattern)) {
            throw new Error(
                `The ${seen.get(pattern)} and ${file} routes clash`
            );
        }
    ...
    function walk(dir, parent_segments, parent_params, stack) {
        const items = fs
            ...
            .map((basename) => {
                ...
                const route_suffix = basename.slice(
                    basename.indexOf('.'),
                    -ext.length
                );
        ...
        items.forEach((item) => {
            ...
            params.push(
                ...item.parts.filter((p) => p.dynamic).map((p) => p.content)
            );
            ...
            if (item.is_dir) {
                ...
                endpoints.push({
                    ...
                    pattern,
                    file: item.file,
                    url: `/${config.appDir}/routes/${item.file.replace(
                        /\.\w+$/,
                        '.js'
                    )}`,
                    params,
    ...
    return {
        assets: fs.existsSync(config.files.assets)
            ? list_files(config.files.assets, '')
            : [],
        layout,
        error,
        components,
