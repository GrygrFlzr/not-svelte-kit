    ...
    async function visit(path) {
        ...
        const rendered = await app.render(
            ...
            },
            {
                ...
                get_static_file: (file) =>
                    readFileSync(join(config.files.assets, file)),
            }
        );
        ...
        if (rendered) {
            ...
            if (response_type === REDIRECT) {
                ...
                fs.writeFileSync(
                    file,
                    `<meta http-equiv="refresh" content="0;url=${encodeURI(
                        location
                    )}">`
                );
            ...
            if (is_html && config.prerender.crawl) {
                ...
                while ((match = pattern.exec(cleaned))) {
                    ...
                    for (const href of hrefs) {
                        ...
                        await visit(
                            parsed.pathname.replace(config.paths.base, '')
                        );
                    }
                }
            }
    ...
    for (const entry of config.prerender.pages) {
        if (entry === '*') {
            ...
            const entries = glob('**/*.svelte', { cwd: config.files.routes })
                .map((file) => {
                    // fix for windows glob results
                    const parts = file.split(/[\\/]/); // TODO is this true for glob results on windows? (no, it is not.)

                    if (
                        parts.some((part) => part[0] === '_' || /\[/.test(part))
                    ) {
                        return null;
                    }
