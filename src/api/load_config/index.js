...
function validate(definition, option, keypath) {
    ...
    for (const key in definition) {
        ...
        const has_children =
            expected.default &&
            typeof expected.default === 'object' &&
...
function resolve(from, to) {
    ...
    return (
        remove_trailing_slash(url.resolve(add_trailing_slash(from), to)) || '/.'
    );
}
...
export async function load_config({ cwd = process.cwd() } = {}) {
    ...
    // this line is the fix for windows
    const config_url = url.pathToFileURL(config_file);
...
export function validate_config(config) {
        ...
        if (!expected.has(key)) {
            warn(
                `Unexpected option ${key}${
                    key in options ? ` (did you mean kit.${key}?)` : ''
                }`
            );
        }
    }
    ...
    validated.paths.assets = resolve(
        validated.paths.base,
        validated.paths.assets
    );
